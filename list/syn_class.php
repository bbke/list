...
{	
	...
	function __construct()
	{
		...
		<table class="synTable">
			<tr class="synTopTr">
				<td class="synTopTd"><a href="?sort=filetype&in=<?php if((isset($_GET["sort"]) && $_GET["sort"] == "filetype" && isset($_GET["in"]) && $_GET["in"] == "asc") || !$_GET){ echo "desc"; }else{ echo "asc"; } ?>"><b><?php echo $lang["T"]; ?></b></a></td>
			...
			foreach($array[0] as $dirData)
			{
				...
				if(isset($config["show_last_modified"]) && $config["show_last_modified"] == true)
				{
					...
					<td class="synFolderTd" align="right">
						...
						echo date("Y-m-d H:i.s", filemtime($dirData)); ?>
					</td>
					<?php
				}
			...
			foreach($array[1] as $fileKey => $fileData)
			{
				...
				if(isset($config["show_last_modified"]) && $config["show_last_modified"] == true)
				{
					...
					<td class="synFileTd" align="right">
						<?php date_default_timezone_set('PRC');
						echo date("d-M-Y H:i.s", filemtime($fileData)); ?>
					</td>
					<?php
				}
	...
	function isEntryInBlacklist($entry)
	{
		...
		if($entry == "." || $entry == ".." || $entry == "index.php" || $entry == "config.php" || $entry == "syn_class.php" || 
		   $entry == "syn_style.css" ||$entry == "folder.png" || $entry == "page_white.png" || $entry == ".openshift" || 
		   $entry == "*.zip" || $entry == "1" || $entry == "build.txt")
		{
			return true;
		}
